...
class CustomMask():
    def __init__(self, input_tensor, device="cpu"):
        B, L, K = input_tensor.shape
       #import ipdb; ipdb.set_trace()
        mask_shape = [B, 1, L, L]
        self._mask = torch.zeros(mask_shape, dtype=torch.bool, device=device)
        
        # Convert input tensor to boolean mask along L dimension
        boolean_mask = input_tensor.any(dim=-1)  # Shape: [B, L]
        
        # Create causal mask (lower triangular)
        causal_mask = torch.triu(torch.ones(L, L, dtype=torch.bool, device=device), diagonal=1)
        
        # Combine causal mask with the input boolean mask
        for b in range(B):
            batch_mask = boolean_mask[b].unsqueeze(0).expand(L, -1)
            combined_mask = causal_mask | ~batch_mask
            self._mask[b, 0, :, :] = combined_mask
        ...
        if self.activation == 'softmax':
            if self.mask_flag:
                if attn_mask is None:
                    ...
                    attn_mask = CustomMask(attn_mask, queries.device)
